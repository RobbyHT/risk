...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('risk_hazop_sub_devices', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('device_id')->nullable()->comment('設備id');
            $table->String('node')->nullable()->comment('節點');
            $table->String('cover_device')->nullable()->comment('涵蓋設備');
            $table->String('node_description')->nullable()->comment('節點功能描述');
            $table->text('process_deviation')->nullable()->comment('製程參數偏離');
    ...
    {
        Schema::dropIfExists('risk_hazop_sub_devices');
    }
};
