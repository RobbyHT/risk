          ...
          <table class="mt-3 table table-bordered border-dark align-middle text-center fw-bold">
            ...
            <tbody class="text-start">
              <tr v-for="(data, i) in deviceList" :id="i" :key="data.id">
                <!-- 1.設備、組件或元件 -->
                <td class="text-center">{{data.sub_device_number}}</td>
                <td class="text-center">{{data.sub_device_name}}</td>
                <td>{{data.sub_device_action}}</td>
                <td>{{data.sub_device_spec}}</td>
                <!-- 2.危害類型、原因與影響 -->
                <td v-show="harmShow">{{data.failure_mode}}</td>
...
<script>
  ...
  export default {
    ...
    methods: {
      getData: function() {
        var vm = this;
        axios.get(`/api/FMEA/subDevice`, { params: { id: vm.id } })
        .then(function (resp) {
