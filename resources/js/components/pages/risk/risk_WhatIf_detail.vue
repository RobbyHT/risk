          ...
          <table class="mt-3 table table-bordered border-dark align-middle text-center fw-bold">
            ...
            <tbody class="text-start">
              <tr v-for="(data, i) in deviceList" :id="i" :key="data.id">
                ...
                <!-- 2.危害類型、原因與影響 -->
                <td v-show="harmShow">{{data.in_case}}</td>
                <td v-show="harmShow">{{data.hazard_type}}</td>
                <td v-show="harmShow">{{data.consequences}}</td>
                <!-- 3.現有防護設施 -->
...
<script>
  ...
  export default {
    ...
    methods: {
      getData: function() {
        var vm = this;
        axios.get(`/api/WhatIf/subDevice`, { params: { id: vm.id } })
        .then(function (resp) {
