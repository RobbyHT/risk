    ...
    <header>
      ...
      <div class="m-3 d-flex align-items-center">
        ...
        <nav class="ms-3" aria-label="breadcrumb">
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><inertia-link href="/risk/FMEA">FMEA列表</inertia-link></li>
            <li class="breadcrumb-item active" aria-current="page">FMEA分析結果</li>
          </ol>
        </nav>
      </div>
    </header>
    <main>
      <div class="m-3 p-2 border border-2 outer-border-color rounded-3">
        <div class="text-center">
          <h2 class="mb-0 fw-bold">FMEA分析結果</h2>
        </div>
        ...
        <div class="table-responsive">
          <table class="sorting table table-bordered border-dark align-middle text-center fw-bold">
            <thead class="align-middle">
              ...
              <tr>
                <!-- 1.設備、組件或元件 -->
                <td class="th-color-1" width="120px">設備編號</td>
                <td class="th-color-1" width="200px">系統或設備</td>
                <td class="th-color-1" width="250px">組件或元件名稱<br/>(製程異常或動作)</td>
                <td class="th-color-1" width="300px">功能規格</td>
                <!-- 2.危害類型、原因與影響 -->
                <td v-show="harmShow" class="th-color-2" width="200px">失效模式</td>
                <td v-show="harmShow" class="th-color-2" width="250px">失效原因<br/>(八大方向思考)</td>
                <td v-show="harmShow" class="th-color-2" width="310px">可能造成影響之情境描述</td>
                <!-- 3.現有防護設施 -->
            ...
            <tbody class="text-start dropzone-users">
              <tr class="tr_clone" v-for="(data, i) in deviceList" :id="i" :key="data.id">
                ...
                <!-- 1.設備、組件或元件 -->
                <td class="text-center"><input type="text" class="form-control" v-model="data.sub_device_number" /></td>
                <td class="text-center"><input type="text" class="form-control" v-model="data.sub_device_name" placeholder="請輸入內容" /></td>
                <td><input type="text" class="form-control" v-model="data.sub_device_action" placeholder="請輸入內容" /></td>
                <td><input type="text" class="form-control" v-model="data.sub_device_spec" placeholder="請輸入內容" /></td>
                <!-- 2.危害類型、原因與影響 -->
                <td v-show="harmShow"><input type="text" class="form-control" v-model="data.failure_mode" placeholder="請輸入內容" /></td>
  ...
  export default {
    ...
    methods: {
      getData: async function() {
        var vm = this;
        await axios.get(`/api/FMEA/subDevice`, { params: { id: vm.id } })
        .then(function (resp) {
      ...
      del: function() {
        ...
        vm.$swal.fire({
          ...
          if (result.value) {
            ...
            axios.post(`/api/FMEA/subDevice/0`, {
              _method: 'DELETE',
              ids: selected
            })
      ...
      changeShow: function() {
        ...
        var tableWidth = 3700;
        if(!this.harmShow) tableWidth -= 760;
