    ...
    <header>
      ...
      <div class="m-3 d-flex align-items-center">
        ...
        <nav class="ms-3" aria-label="breadcrumb">
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><inertia-link href="/risk/WhatIf">What-If列表</inertia-link></li>
            <li class="breadcrumb-item active" aria-current="page">What-If分析結果</li>
          </ol>
        </nav>
      </div>
    </header>
    <main>
      <div class="m-3 p-2 border border-2 outer-border-color rounded-3">
        <div class="text-center">
          <h2 class="mb-0 fw-bold">What-If分析結果</h2>
        </div>
          ...
          <table class="sorting table table-bordered border-dark align-middle text-center fw-bold">
            <thead class="align-middle">
              <tr>
                ...
                <td class="head head-bg-1" colspan="2">1.設備、組件或元件</td>
                <td v-show="harmShow" class="head head-bg-2" colspan="3">2.危害類型、原因與影響</td>
              ...
              <tr>
                <!-- 1.設備、組件或元件 -->
                <td class="th-color-1" width="100px">流水號</td>
                <td class="th-color-1" width="390px">系統或設備或情境描述</td>
                <!-- 2.危害類型、原因與影響 -->
                <td v-show="harmShow" class="th-color-2" width="320px">萬一</td>
                <td v-show="harmShow" class="th-color-2" width="200px">危害類型</td>
                <td v-show="harmShow" class="th-color-2" width="320px">後果</td>
                <!-- 3.現有防護設施 -->
                <td v-show="protectShow" class="th-color-3" width="300px">工程控制<br/>(含偵測或連鎖)</td>
            ...
            <tbody class="text-start dropzone-users">
              <tr class="tr_clone" v-for="(data, i) in deviceList" :id="i" :key="data.id">
                ...
                <!-- 1.設備、組件或元件 -->
                <td class="text-center">{{sortArr.indexOf(i.toString())+1}}</td>
                <td><input type="text" class="form-control" v-model="data.description" placeholder="請輸入內容" /></td>
                <!-- 2.危害類型、原因與影響 -->
                <td v-show="harmShow"><input type="text" class="form-control" v-model="data.in_case" placeholder="請輸入內容" /></td>
                <td v-show="harmShow"><input type="text" class="form-control" v-model="data.hazard_type" placeholder="請輸入內容" /></td>
                <td v-show="harmShow"><input type="text" class="form-control" v-model="data.consequences" placeholder="請輸入內容" /></td>
                <!-- 3.現有防護設施 -->
    ...
    mounted() {
      ...
      $(".dropzone-users").sortable({
        ...
        update: function(event, ui) {
          vm.sortArr = $(this).sortable('toArray');
        }
      });
    }, 
    methods: {
      getData: async function() {
        var vm = this;
        await axios.get(`/api/WhatIf/subDevice`, { params: { id: vm.id } })
        .then(function (resp) {
        ...
        vm.sortArr = $(".dropzone-users").sortable("toArray");
      },
      selectAll: function() {
        const vm = this;
        vm.selectedList = [];
      ...
      add: async function() {
        await this.deviceList.push({...this.deviceData});
        this.sortArr = $(".dropzone-users").sortable("toArray");
      },
      del: function() {
        const vm = this;
        vm.$swal.fire({
          ...
          if (result.value) {
            ...
            axios.post(`/api/WhatIf/subDevice/0`, {
              _method: 'DELETE',
              ids: selected
            })
      ...
      save: function() {
        const vm = this;
        axios.post(`/api/WhatIf/subDevice`, {
      ...
      changeShow: function() {
        ...
        var tableWidth = 3500;
        if(!this.harmShow) tableWidth -= 840;
        if(!this.protectShow) tableWidth -= 750;
