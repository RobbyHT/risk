            ...
            <tbody>
              <tr v-for="(data, i) in deviceList" :key="data.id">
                ...
                <td class="text-center" data-th="檢視與編輯">
                  <inertia-link :href="'/risk/FMEA/detail/'+data.id" class="btn btn-primary mx-1">檢視</inertia-link>
                  <inertia-link :href="'/risk/FMEA/edit/'+data.id" class="btn btn-warning mx-1">編輯</inertia-link>
                </td>
              </tr>
            </tbody>
          </table>
  ...
  export default {
    ...
    methods: {
      getData: async function() {
        ...
        await axios.get(`/api/FMEA/device`)
        .then(function (resp) {
